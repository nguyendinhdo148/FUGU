...
export const VIPLounge = () => {
  ...
  const vipLoungeDetails = {
    name: "Maxim Saigon VIP Private Lounge",
    capacity: "20-30 persons",
    description:
      "Exclusive private space for VIP events and special occasions at Maxim Saigon. Sophisticated ambiance with premium amenities.",
    features: [
    ...
    note:
      "Ideal for VIP gatherings, corporate meetings, and special celebrations. Contact for availability.",
  };

  // SEO: Description động
  const seoDescription = `Maxim Saigon VIP Private Lounge - Phòng VIP riêng tư tại Maxim Saigon. Sức chứa 20-30 người, tầng 2, màn hình 4K 100 inch, sound system cao cấp. Đặt bàn: 085 587 3979`;
  ...
  const handlePrev = () => {
    setSelectedImage(
      (prev) => (prev - 1 + vipLoungeImages.length) % vipLoungeImages.length
    );
    setAutoSlide(false);
  };
  ...
  return (
    <section
      ...
      aria-label="Maxim Saigon VIP Private Lounge - Phòng VIP riêng tư"
    >
      {/* SEO: Hidden semantic info */}
      <meta itemProp="name" content="Maxim Saigon VIP Private Lounge" />
      <meta itemProp="description" content={seoDescription} />
      <meta itemProp="occupancy" content="20-30" />
      <meta itemProp="floorLevel" content="2" />
      <meta
        itemProp="amenityFeature"
        content="Private Space, Sound System, 4K Screen, VIP Service"
      />
      ...
      <header className="text-center mb-6 md:mb-8">
        <div className="inline-flex items-center gap-2 md:gap-3 mb-3 md:mb-4">
          ...
          <h3
            ...
            itemProp="name"
          >
            Maxim Saigon VIP Private Lounge
          </h3>
      ...
      <div className="grid lg:grid-cols-2 gap-6 md:gap-8">
        ...
        <div className="space-y-3 md:space-y-4">
          <figure
            className="relative rounded-xl md:rounded-2xl overflow-hidden shadow-lg border border-border group"
            itemProp="image"
            itemScope
          ...
          >
            ...
            >
              <img
                ...
                alt={`Maxim Saigon VIP Private Lounge - Phòng VIP tầng 2 tại Maxim Saigon - Ảnh ${
                  selectedImage + 1
                }`}
              ...
              <meta
                itemProp="name"
                content="Maxim Saigon VIP Private Lounge - Phòng VIP tầng 2"
              />
            ...
            <div className="absolute bottom-0 left-0 right-0 bg-gradient-to-t from-black/70 to-transparent p-2 md:p-4">
              <div className="flex items-center justify-between">
                <div>
                  <h4 className="text-sm md:text-lg font-bold text-white font-notoserif">
                    {vipLoungeDetails.name}
                  </h4>
                  <p className="text-white/80 text-[10px] md:text-sm font-inter">
                    View {selectedImage + 1} of {vipLoungeImages.length}
                  </p>
                </div>
                    ...
                    {vipLoungeImages.map((_, index) => (
                      <button
                        ...
                        className={`h-1 md:h-2 rounded-full transition-all duration-300 ${
                          selectedImage === index
                            ? "bg-white w-2 md:w-4"
                            : "bg-white/40 w-1 md:w-2 hover:bg-white/60"
                        }`}
            ...
            <div className="absolute top-2 right-2 md:top-3 md:right-3">
              <button
                ...
                aria-label={autoSlide ? "Tạm dừng slideshow" : "Phát slideshow"}
              >
                {autoSlide ? "⏸️" : "▶️"}
              </button>
            </div>
          ...
          <div className="grid grid-cols-5 gap-1 md:gap-2">
            {vipLoungeImages.map((image, index) => (
              ...
              >
                <img
                  src={image}
                  alt={`Maxim Saigon VIP Private Lounge - Ảnh thu nhỏ ${index + 1}`}
        ...
        {/* ==================== Details Section ==================== */}
        <div className="space-y-3 md:space-y-4">
          <article className="bg-card rounded-xl p-4 md:p-5 border border-border shadow-sm">
            <div className="flex items-start gap-2 md:gap-3 mb-2 md:mb-3">
              <div>
                <h4
                  className="text-base md:text-lg font-bold text-foreground font-notoserif"
                  itemProp="name"
                >
                  {vipLoungeDetails.name}
                </h4>
                <p
                  className="text-muted-foreground text-xs md:text-sm font-inter line-clamp-2"
                  itemProp="description"
                >
                  {vipLoungeDetails.description}
                </p>
              </div>
            </div>
            <div className="mt-2 md:mt-3 p-2 md:p-3 bg-secondary rounded-lg">
              <div className="flex items-center justify-around gap-2 md:gap-6">
                <div className="text-center">
                  <div className="text-base md:text-xl font-bold text-foreground font-inter">
                    20-30
                  </div>
                  <div className="text-[10px] md:text-xs text-muted-foreground font-inter">
                    Persons
                  </div>
                </div>
                <div className="text-center">
          ...
          <section
            ...
            aria-label="Tiện ích VIP Lounge"
          >
            <h4 className="text-base md:text-lg font-bold text-foreground mb-2 md:mb-3 font-notoserif">
              Features
            </h4>
            <ul className="grid grid-cols-2 gap-1 md:gap-2 list-none p-0 m-0">
              {vipLoungeDetails.features.map((feature, index) => (
                <li
                  key={index}
                  className="flex items-center gap-1 md:gap-2 p-1.5 md:p-2 bg-secondary rounded-lg"
                >
                  <div
                    className="w-5 h-5 md:w-6 md:h-6 rounded-md bg-purple-500/10 flex items-center justify-center flex-shrink-0"
                    aria-hidden="true"
                  >
                    <span className="text-purple-600 dark:text-purple-400 text-[10px] md:text-xs">
                      ✓
                    </span>
                  </div>
                  <span className="text-[10px] md:text-sm text-foreground font-inter truncate">
                    {feature}
                  </span>
                </li>
              ))}
          ...
          <aside className="mt-3 md:mt-4 p-2 md:p-3 bg-amber-500/10 border border-amber-500/20 rounded-lg">
            <div className="flex items-start gap-1 md:gap-2">
              <div className="flex-shrink-0 mt-0.5">
                <div
                  className="w-4 h-4 md:w-5 md:h-5 rounded-full bg-amber-500/20 flex items-center justify-center"
                  aria-hidden="true"
                >
                  <span className="text-amber-600 dark:text-amber-400 text-[8px] md:text-xs">
                    ⚠️
                  </span>
                </div>
              </div>
              <div>
                <p className="text-amber-800 dark:text-amber-400 text-[8px] md:text-xs font-inter">
                  <span className="font-semibold">Decoration Notice:</span>{" "}
                  Floral decorations shown are for demonstration only. Actual
                  decorations require purchasing a decor package.
                </p>
              </div>
            </div>
          </aside>
          ...
          <a
            ...
            className="w-full py-2 md:py-3 bg-purple-600 hover:bg-purple-700 dark:bg-purple-700 dark:hover:bg-purple-600 text-white rounded-lg font-semibold transition-all duration-300 shadow-md hover:shadow-lg flex items-center justify-center gap-1 md:gap-2"
            aria-label="Liên hệ đặt Maxim Saigon VIP Private Lounge"
          >
            <svg
              className="w-4 h-4 md:w-5 md:h-5"
              fill="none"
            ...
            <span className="text-xs md:text-base font-inter">
              Contact for VIP Lounge
            </span>
          </a>
        </div>
      </div>
        ...
        >
          ...
          <div className="relative w-full max-w-6xl h-[60vh] md:h-[80vh]">
            <img
              ...
              alt={`${zoomedImage.name} - Ảnh phóng to`}
          ...
          <div className="absolute bottom-4 left-1/2 transform -translate-x-1/2 z-20">
            <div className="bg-black/60 backdrop-blur-sm text-white/80 px-2 py-1 md:px-4 md:py-2 rounded-full border border-white/20 flex items-center gap-1 md:gap-2">
              <span className="text-amber-400 text-xs md:text-sm" aria-hidden="true">
                ⚠️
              </span>
              <span className="text-[8px] md:text-sm font-inter">
                Floral decor requires separate package
              </span>
            </div>
          </div>
